...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.groupBy(c=>c).map({ case (char, string) => (char, string.length) }).toList.sorted
   ...
   */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x.toMap.foldLeft(Map[Char, Int]())((accum, pair) => {
    accum.updated(pair._1, pair._2 - y.toMap.withDefaultValue(0)(pair._1))
